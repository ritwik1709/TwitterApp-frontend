...
const MainPage = ({ user }) => {
    ...
    useEffect(() => {
        fetch(`https://twitterapp-backend-n4d9.onrender.com/userPost?email=${user?.email}`)
            .then(res => res.json())
            .then(data => {
                setPosts(data)
            })
    ...
    const handleUploadCoverImage = (e) => {
        ...
        axios.post('https://api.imgbb.com/1/upload?key=ad3f181e19593ca55595943a22c59a7f', formData)
            .then(res => {
                ...
                setisLoading(false);
                if(url){
                    axios.patch(`https://twitterapp-backend-n4d9.onrender.com/userUpdates/${user?.email}`,userCoverImage)
                }
            })
            .catch((error) => {
                console.log(error)
    ...
    const handleUploadProfileImage = (e) => {
        ...
        axios.post('https://api.imgbb.com/1/upload?key=ad3f181e19593ca55595943a22c59a7f', formData)
            .then(res => {
                ...
                setisLoading(false);
                if(url){
                    axios.patch(`https://twitterapp-backend-n4d9.onrender.com/userUpdates/${user?.email}`,userProfileImage)
                }
            })
            .catch((error) => {
                console.log(error)
